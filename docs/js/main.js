$(function(){
    ...
    const $episode_track = $episode.find('.episode-flex'); // 에피소드 가로 스크롤 해야하는 영역
    ...
    $('.characters>div>div').click(function(){ // 마우스 클릭하면 캐릭터 상세 설명 나타나기(캐릭터 목록 아래로 배치)
    ...
    $('#close-btn').click(function(event){ // 뒤로가기 버튼 클릭 시 캐릭터 설명창 사라지기
    ...
    /* gallery-container */
    $('.progress>.star').click(function(){ // 갤러리 별 클릭
    ...
    function updateSectionMetrics() {
        /* episode */
        const episodeTrackWidth = $episode_track.get(0).scrollWidth; // 트랙의 실제 가로 총길이
        // 섹션 높이 = (가로총길이 - 뷰포트너비) + 뷰포트높이
        // 이렇게 해야 사용자가 세로로 스크롤할 때, 그 구간 동안 가로 이동이 완주됨
        const episodeSectionHeight = (episodeTrackWidth - device_width) + device_height;
        $episode.height(episodeSectionHeight);
        
        // 스크롤 시 사용할 값 저장
        $episode.data('episodeTrackWidth', episodeTrackWidth);
        $episode.data('maxX', Math.max(0, episodeTrackWidth - device_width));
    ...
    function onScrollPrologue(){ /* prologue scroll */
        const $prologue_info = getSectionInfo($prologue);
        ...
        $(".prologue-box>.logo").css({ // 프롤로그 로고 작아지기
            transform: `scale(${progress})`,
        });
        
        $(".prologue-bg>div").each(function(index){ // 스크롤하면 에피소드에 오브젝트 움직이기
            if(index <= 4){
                $(this).css({
    ...
    function onScrollEpisode() { /* episode scroll */
        const $episode_info = getSectionInfo($episode);
        if (!checkScroll($episode_info)) return; // 스크롤 범위 밖이면
        
        const viewportH = $(window).height();
        const maxX = $episode.data('maxX') || 0;
        const scrolledIn = scrollTop - $episode_info.secTop; // 섹션 진입 후 진행된 세로 스크롤량
        const progress = Math.min(Math.max(scrolledIn / ($episode_info.secH - viewportH), 0), 1); // 진행도(0 > 1), 그에 따른 가로 이동량
        const translateX = - (maxX * progress);
        $episode_track.css('transform', `translateX(${translateX}px)`); // 가로 이동 적용
        $episode.find('.object01').each(function(index){ // 스크롤하면 사진 회전하기
            let move = Math.min(Math.max((scrolledIn - (device_width * index)) / (device_width/2), 0), 1); // 진행도(0 > 1), 그에 따른 가로 이동량
            console.log(move);
            $(this).css('transform', `rotateY(${180*move}deg)`);
        })
    }

    function onScrollGallery() { /* gallery scroll */
        const $gallery_info = getSectionInfo($gallery);
