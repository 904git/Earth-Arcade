$(function () {
    ...
    function writeGallery() {
        ...
        for (let i = 1; i <= 10; i++) {
            ...
            let li = $('<li>').attr('class', 'gallery-img').attr('id', `img${i}`);
            img.appendTo(li);
    ...
    function writeBackground() {
        ...
        for (let i = 1; i <= 4; i++) {
            for (let j = 1; j <= 5; j++) {
                let object = $('<div>').attr('class', `object0${j}`).attr('data-speed', `0.${j}`);
                object.appendTo($(`.background-object>.bg${i}`));
            }
        }
    }
    /* gsap */
    ...
    gsap.to(sections, {
        ...
        scrollTrigger: {
            ...
            end: () => '+=' + 600 * $('.gallery-img').length,
        },
    });
    let objects = gsap.utils.toArray('.gallery-bg>div');
    gsap.set(objects, { opacity: 0, scale: 0.3, y: -50 });
